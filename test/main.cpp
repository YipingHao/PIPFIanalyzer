...
int static DataMatrixSwitch(hyperlex::dictionary&dict, const char* outputPath, analyzer::FIexpresses & expressions)
{
    ...
    if (outputFile != NULL) {
        // 写入表头信息
        fprintf(stdout, "# PIP values generated by FIanalyzer\n");
        fprintf(stdout, "# Number of data points: %d\n", row);
        fprintf(stdout, "# Number of PIP polynomials: %zu\n", expressions.getItems().size());
        if (hasEnergy) {
            fprintf(stdout, "# Including energy values\n");
        }
        fprintf(stdout, "\n");
